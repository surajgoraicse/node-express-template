...
export const authenticateUser = catchAsync(async (req, _res, next) => {
    const authHeader = req.headers.authorization;
    // Check for Bearer token
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        logger.warn('[AUTH_MIDDLEWARE] No Bearer token found in Authorization header');
    ...
    try {
        ...
        if (!user) {
            throw new APIError(401, 'Unauthorized. User associated with this token not found.');
        }
        // Exclude password from the user object attached to the request
        const { password, ...userWithoutPassword } = user;
        // Attach user to the request object
        req.user = userWithoutPassword;
        logger.info(`[AUTH_MIDDLEWARE] User authenticated successfully: ${user.id}`);
        next();
    }
    catch (error) {
        ...
        throw new APIError(500, 'An unexpected error occurred during user authentication.');
    }
});
